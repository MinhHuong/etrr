"""Script to train an LSTM-AE for R2N data"""
import sys
...
# no touching here
cycpath = 'E:\\PhD\\Data\\clean\\2021\\R2N_sample'
testpath = 'E:\\PhD\\Data\\clean\\2021\\R2N_testAE'
cyctypes = ['o', 'f']
...
def offline_training(model, X_train_, y_train_, X_val_, y_val_):
    """
    TRAIN AN OFFLINE MODEL

    :return:
    """
    train_history = model.fit(X_train_, y_train_, validation_data=(X_val_, y_val_),
                              batch_size=32, epochs=epochs, verbose=1)
    return model, train_history, []


def online_training(model, X_train_, y_train_, is_incremental):
    """
    ...
    for i in range(N):
        if is_incremental:
            ...
            i_loss = model.train_on_batch(x, y, reset_metrics=False)
        else:
        ...
        for _, mt in enumerate(model.metrics_names):
            if mt not in losses.keys():
                losses[mt] = [i_loss]
            else:
                losses[mt].append(i_loss)
        ...
        feat = np.squeeze(enc_.predict(x))
...
if __name__ == '__main__':
    ...
    with open('r2n_training_data.pkl', mode='rb') as f:
        res = pickle.load(f)
    ...
    for ctype in cyctypes:
        print(f'\n{ctype.upper()} CYCLES\n')
        metrics = {'dataset': 'r2n', 'mode': training_mode, 'cycle_type': ctype, 'n_clean_cycles': len(X[ctype])}
        ...
        joint_model = kr.models.Model(input_cyc, decoder(enc, timesteps, dim, ctype),
                                      name=f'r2n-{ctype}-plain-model-{training_mode}')
        joint_model.compile(loss='mean_squared_error', optimizer=kr.optimizers.Adam(learning_rate=0.0001)
        )
        joint_model.summary()
        # draw the model
        tf.keras.utils.plot_model(
            joint_model, show_shapes=True, show_layer_names=True,
            to_file=f'./plots/r2n/plain_310123/r2n-{ctype}-model-40ft-{training_mode}.png',
        )

        # SPLIT TO TRAINING & VALIDATION SET -------------------------------------
        X_train, X_val, y_train, y_val = train_test_split(X[ctype], X[ctype], test_size=0.2, random_state=seed)
        ...
        if training_mode == 'offline':
            joint_model, history, train_features = offline_training(joint_model, X_train, y_train, X_val, y_val)
        # ONLINE TRAINING: BY INSTANCE OR INCREMENTALLY
        else:
            if training_mode == 'online':
                joint_model, history, train_features = online_training(joint_model, X_train, y_train,
        ...
        with open(f'./logs/r2n/plain_310123/train_history_{ctype}_{training_mode}.pkl', mode='wb') as f:
            if training_mode == 'offline':
        ...
        recons = joint_model.predict(cyc.reshape(1, cyc.shape[0], cyc.shape[1]))
        recons = np.squeeze(recons)
        ax.plot(recons)
        ax.set_title('Reconstructed', fontsize=16)
        # save the figure
        plt.savefig(f'./plots/r2n/plain_310123/r2n-{ctype}-results-40ft-{training_mode}.png')
        plt.close()
        ...
        """
        print('\nSaving the model...')
        model_path = f'./models/r2n/plain_310123/{ctype}/{training_mode}'
        ...
        for f in os.listdir(testpath):
            ...
            test_cyc, test_ctype = test_df[atts].values, f.replace('.pkl', '')[-1]
            if ctype == test_ctype:
        ...
        np.save(f'./features/r2n/plain_310123/r2n-{ctype}-features-{training_mode}.npy', features)
        ...
        if training_mode != 'offline':
            ...
            np.save(f'./features/r2n/plain_310123/r2n-{ctype}-trainfeatures-{training_mode}.npy', train_features)
        ...
        metrics_fname = f'./logs/r2n/plain_310123/metrics_40ft_{training_mode}.csv'
