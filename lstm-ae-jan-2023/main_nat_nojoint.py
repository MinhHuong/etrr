...
cyctypes = ['o', 'f']
atts = ['sampled_position', 'sampled_voltage', 'sampled_current', 'sampled_fdcf', 'sampled_fdcv']
...
if __name__ == '__main__':
    ...
    for ctype in cyctypes:
        ...
        joint_model = kr.models.Model(input_cyc, decoder(enc, timesteps, dim, ctype),
                                      name=f'nat-{ctype}-plain-model-{training_mode}')
        ...
        tf.keras.utils.plot_model(
            ...
            to_file=f'./plots/nat/plain_310123/nat-{ctype}-model-40ft-{training_mode}.png',
        )
        ...
        with open(f'./logs/nat/plain_310123/train_history_{ctype}_{training_mode}.pkl', mode='wb') as f:
            if training_mode == 'offline':
        ...
        # save the figure
        plt.savefig(f'./plots/nat/plain_310123/nat-{ctype}-results-40ft-{training_mode}.png')
        plt.close()
        ...
        """
        print('\nSaving the model...')
        model_path = f'./models/nat/plain_310123/{ctype}/{training_mode}'
        ...
        np.save(f'./features/nat/plain_310123/nat-{ctype}-features-{training_mode}.npy', features)

        # if training mode is online/online-incremental, also save the features extracted while training
        if training_mode != 'offline':
            train_features = np.array(train_features, dtype=object)
            print('Training features:', train_features.shape)
            np.save(f'./features/nat/plain_310123/nat-{ctype}-trainfeatures-{training_mode}.npy', train_features)
        ...
        metrics_fname = f'./logs/nat/plain_310123/metrics_40ft_{training_mode}.csv'
