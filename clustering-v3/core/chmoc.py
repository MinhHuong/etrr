...
# from core.denstream_original import DenStream
...
class CHMOC:
    ...
    CHMOC uses an online clustering algorithm as the core processor.
    ...
    def __init__(self, config, db_conn):
        ...
        self.clusterer = DenStream(eps=config['clustering']['eps'],
                                   ...
                                   W=W,
                                   time_unit=config['clustering']['time_unit'],
    ...
    def process(self, batch: List[Instance]):
        ...
        # update the parameters (new speed is the number of instances in this batch)
        # self.clusterer.update_parameters(new_v=len(batch))

        # (1) update DenStream on every point in this batch
        start = time.perf_counter()
        for idx, inst in enumerate(batch):
            # if idx + 1 % 1000 == 0:
            #     print(f'\tInstance {inst.id} ({idx+1}/{len(batch)})')
            self.clusterer.process(inst)
        end = time.perf_counter()
        self.denstream_time = end - start

        # (2) once the whole batch has been processed, perform pruning
        # start = time.perf_counter()
        # self.clusterer.prune(t_upd=batch[-1].tsp)  # t = timestamp of the last instance in the batch
        # end = time.perf_counter()
        # self.prune_time = end - start

        # (3) reassess the cluster: find the new reference cluster and recompute the weight
        start = time.perf_counter()
        # self.clusterer.reassess_clusters(t_upd=batch[-1].tsp)
        ...
        for sys_id in sys_ids:
            # self.clusterer.update_health(sys_id=sys_id, t_upd=batch[-1].tsp)
