...
# no touching here
cycpath = '[path to training cycles repo]'
testpath = '[path to testing cycles repo]'
cyctypes = ['o', 'f']
...
def online_training(model, X_train_, y_train_, y_ctxt_train, is_incremental):
    """
...
if __name__ == '__main__':
    ...
    with open('[name of pickle file]', mode='rb') as f:
        res = pickle.load(f)
    ...
    for ctype in cyctypes:
        print(f'\n{ctype.upper()} CYCLES\n')
        metrics = {'dataset': 'ds', 'mode': training_mode, 'cycle_type': ctype, 'n_clean_cycles': len(X[ctype])}
        ...
        joint_model = kr.models.Model(input_cyc,
                                      ...
                                      name=f'{ctype}-joint-model-{training_mode}')
        joint_model.compile(
        ...
        with open(f'./train_history_{ctype}_{training_mode}.pkl', mode='wb') as f:
            if training_mode == 'offline':
        ...
        # save the figure
        plt.savefig(f'./{ctype}-results-40ft-{training_mode}.png')
        plt.close()
        ...
        """
        print('\nSaving the model...')
        model_path = f'./models/{ctype}/{training_mode}'
        ...
        np.save(f'./{ctype}-features-{training_mode}.npy', features)
        ...
        metrics_fname = f'./metrics_40ft_{training_mode}.csv'
