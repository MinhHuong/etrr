...
class InterCEOffline:
    ...
    def train(self, X: pd.DataFrame, X_id):
        """
        Run InterCE in training mode (always return query)
        ...
        # check if this data motif has been seen, and add it if it is not the case
        has_seen, motif_id = self.memory.has_seen_data(x, X_id)  # check and retrieve the motif ID at the same time

        # if this motif has been seen, create a buffered query, but not automatically select cycles
        if has_seen:
            query = self.memory.create_query(X_id, x, candidates)
            self._mark_buffered(query, motif_id)
        else:
            # create a new official query
            query = self.memory.create_query(X_id, x, candidates)
    ...
    def test(self, X: pd.DataFrame, X_id):
        """
        Test InterCE on new input (no more query created)
        ...
        if results is not None:
            ...
            # FIXED record the number of extracted cycles
        ...
        if has_seen:
            ...
            if self.memory.has_processed_query_of(motif_id):
                ...
                if results is not None:
                    ...
                    self.n_auto_cycles += len(results.markers)
                    self.n_auto += 1
            # otherwise, create a buffered query for X
            else:
                query = self.memory.create_query(X_id, x, candidates)
                self._mark_buffered(query, motif_id)
        # if X is new
        ...
        if n_new_feedback > 0:
            ...
            for fid in feedback_ids:
                ...
                for X_id, candidates in buf_queries.items():
                    ...
                    self.n_auto += 1
                    # if results is not None:
                    #     print(f'\tIn SOLVE BUFFERED mode, selected {len(results.markers)} cycles')
                    # else:
                    #     print(f'\tIn SOLVE BUFFERED mode, no results!!!')
                    if results is not None:
                        # self._save_results_from_buffered_queries(X_id=X_id, results=results)
                        self.dbconn.solve_buffered_queries(query_fname=X_id, results=results)
                        # FIXED record the number of extracted cycles
                        self.n_cycles += len(results.markers)
                        self.n_auto_cycles += len(results.markers)
