...
def _motif_distance(a, b) -> float:
    ...
    # compute the euclidean distance (naive approach)
    # dist = np.sqrt(sum([(i - j) * (i - j) for i, j in zip(a_, b_)]))
    return dist
...
class Memory:
    ...
    def __init__(self, sim_thres, query_thres, dbconn):
        """Initialize the memory"""
        self.sim_thres = sim_thres      # similarity threshold for data motifs
        self.query_thres = query_thres  # similarity threshold for queries
...
#     data = [pd.read_pickle(os.path.join(inpath, f))[atts].values for f in files]
#     N = len(files)
#
#     start = time.perf_counter()
#     dist_pyts = np.zeros((N, N))
#     for i in range(N):
#         for j in range(i+1, N):
#             dist_pyts[i, j] = np.mean([dtw(data[i][:, k], data[j][:, k], method='fast') for k in range(len(atts))])
#     end = time.perf_counter()
#     print(f'TOTAL TIME (PYTS): {end-start:.3f} seconds')
#
#     start = time.perf_counter()
#     dist_fastdtw = np.zeros((N, N))
#     for i in range(N):
#         for j in range(i+1, N):
#             dist_fastdtw[i, j] = np.mean([fastdtw(data[i][:, k], data[j][:, k], dist=euclidean)[0]
#                                           for k in range(len(atts))])
#     end = time.perf_counter()
#     print(f'TOTAL TIME (FASTDTW): {end - start:.3f} seconds')
#
#     # plot all test cycles
...
#     sns.heatmap(dist_pyts, ax=ax)
#     ax.set_title('PYTS')
#     ax = fig.add_subplot(1, 2, 2)
#     sns.heatmap(dist_fastdtw, ax=ax)
#     ax.set_title('FAST DTW')
#     plt.show()
