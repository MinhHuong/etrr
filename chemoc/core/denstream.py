...
class MicroCluster:
    ...
    def __init__(self, dim: int, mc_type: IntEnum, t0: float, distance: Callable,
    ...
    def _update_radius(self):
        ...
        """
        # prevent division by 0 which thwarts the computation
        if _is_close_to_0(self.W):
            self.radius = 0
        else:
    ...
    def _compute_radius(self, ls, ss, n) -> float:
        ...
        """
        # with n-1 as working on a sample, get the max std at the end (should it be the mean instead?)
        if n > 1:
            a = ss / (n-1)
            b = (ls * ls) / (n * (n - 1))
        else:
    ...
    def decay(self, last_tsp_per_system):
        ...
        for sys_id in self.cf1.keys():
            ...
            interval = (last_tsp_per_system[sys_id] - self.last_upd[sys_id])
...
class DenStream:
    ...
    def __init__(self, eps: float, mu: float, beta: float, lamb: float, W: float,
    ...
    def _compute_pruning_interval(self) -> float:
        ...
        """
        # if we lump u to u+ -> minital timespan also changes
    ...
    def _reassess_epsilon(self):
        ...
        """
        # from Putina and Rossi's work (k = 3 by default)
    ...
    def warm_start(self, batch: List[Instance]):
        ...
        for i in range(N):
            if not visited[i]:
                ...
                if len(ngb) >= self.outlier_threshold:
                    pmc = MicroCluster(dim=D, mc_type=MCType.PMC, t0=self.T_now, distance=self.distance,
                                       min_points_system=self.min_points_system, decay_factor=self.lamb)
        ...
        for inst in batch:
            ...
            # change this in the database as well?...
    ...
    def _merge(self, inst):
        ...
        if c_p is not None and c_p.recompute_radius(inst.data) <= self.eps:
            ...
            # if it can't be added in the nearest PMC nor the OMC -> create a new OMC
                ...
                new_omc = MicroCluster(dim=inst.dim, mc_type=MCType.OMC, t0=self.T_now, distance=self.distance,
                                       min_points_system=self.min_points_system, decay_factor=self.lamb)
                new_omc.add(inst)
